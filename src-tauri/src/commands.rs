pub mod commands {
    ...
    pub fn get_filter_options(state: State<AppState>) -> Result<FilterOptions, String> {
        ...
        let mut stmt_years = db.prepare("SELECT DISTINCT year_enrolled FROM students ORDER BY year_enrolled DESC").map_err(|e| e.to_string())?;
        let year_iter = stmt_years.query_map([], |row| row.get::<_, u32>(0)).map_err(|e| e.to_string())?;
        for y in year_iter {
            if let Ok(year) = y { years.push(year); }
        }

        let mut programs = Vec::new();
        let mut stmt_progs = db.prepare("SELECT DISTINCT program FROM students ORDER BY program ASC").map_err(|e| e.to_string())?;
        let prog_iter = stmt_progs.query_map([], |row| row.get::<_, String>(0)).map_err(|e| e.to_string())?;
        for p in prog_iter {
            if let Ok(prog) = p { programs.push(prog); }
        }
    ...
    #[tauri::command]
    pub fn get_students(state: State<AppState>, year: Option<u32>, passed: Option<bool>, search: Option<String>, program: Option<String>) -> Result<Vec<Student>, String> {
        let db = state.0.lock().unwrap();
        let mut query = "SELECT student_id, last_name, first_name, middle_name, program, year_enrolled, shs_name, status_passed_sf10 FROM students WHERE 1=1".to_string();
        ...
        let student_iter = stmt.query_map(rusqlite::params_from_iter(params_vec.iter()), |row| {
            Ok(Student {
    ...
    pub fn update_student(state: State<AppState>, student: Student) -> Result<(), String> {
        let db = state.0.lock().unwrap();
        db.execute(
            "UPDATE students SET last_name = ?, first_name = ?, middle_name = ?, program = ?, year_enrolled = ?, shs_name = ?, status_passed_sf10 = ? WHERE student_id = ?",
            params![
                student.last_name,
                student.first_name,
                student.middle_name,
        ...
        db.execute("INSERT INTO logs (action, details) VALUES (?, ?)", params![action, details]).ok();
        Ok(())
    }

    #[tauri::command]
    pub fn toggle_sf10(state: State<AppState>, student_id: String, passed: bool) -> Result<(), String> {
        let db = state.0.lock().unwrap();
        db.execute(
    ...
    #[tauri::command]
    pub fn get_dashboard_stats(state: State<AppState>, year: Option<u32>) -> Result<DashboardStats, String> {
        let db = state.0.lock().unwrap();
        ...
        let year_cond = if let Some(y) = year {
            format!("WHERE year_enrolled = {}", y)
        } else {
            "".to_string()
        };
    ...
    #[tauri::command]
    pub fn get_printable_students(state: State<AppState>, year: Option<u32>, passed: Option<bool>, programs: Vec<String>) -> Result<Vec<Student>, String> {
        let db = state.0.lock().unwrap();
        let mut query = "SELECT student_id, last_name, first_name, middle_name, program, year_enrolled, shs_name, status_passed_sf10 FROM students WHERE 1=1".to_string();
        let mut params_vec: Vec<rusqlite::types::Value> = Vec::new();
        ...
        if !programs.is_empty() {
            let program_placeholders: Vec<String> = programs.iter().map(|_| "?".to_string()).collect();
            query.push_str(&format!(" AND program IN ({})", program_placeholders.join(",")));
            for p in programs {
    ...
    #[tauri::command]
    pub fn import_students(state: State<AppState>, students: Vec<Student>, file_name: String) -> Result<usize, String> {
        ...
        for student in students {
            let res = tx.execute(
                "INSERT OR REPLACE INTO students (student_id, last_name, first_name, middle_name, program, year_enrolled, shs_name, status_passed_sf10, import_batch_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                params![
                    student.student_id,
                    student.last_name,
                    student.first_name,
    ...
    pub fn delete_import_batch(state: State<AppState>, id: u32) -> Result<(), String> {
        ...
        let students_deleted = db.execute(
    ...
    pub fn get_logs(state: State<AppState>) -> Result<Vec<LogEntry>, String> {
        let db = state.0.lock().unwrap();
        let mut stmt = db.prepare("SELECT id, timestamp, action, details FROM logs ORDER BY id DESC LIMIT 100").map_err(|e| e.to_string())?;
