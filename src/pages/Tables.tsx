...
export default function Tables() {
  ...
  const [yearCode, setYearCode] = useState<number | undefined>(undefined);
  ...
  const fetchStudents = async () => {
    setLoading(true);
    try {
      const data = await api.getStudents(yearCode, statusFilter, search, programFilter || undefined);
      setStudents(data);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
          ...
          <select 
            value={yearCode || ""}
            onChange={(e) => setYearCode(e.target.value ? Number(e.target.value) : undefined)}
                ...
                return (
                  <tr key={student.student_id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <button 
                        onClick={() => toggleStatus(student.student_id, student.status_passed_sf10)}
