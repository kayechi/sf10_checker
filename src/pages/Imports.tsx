...
export default function Imports() {
  ...
  const handleDeleteBatch = async (id: number) => {
    try {
      ...
      setStatus({ type: 'success', message: 'Import batch deleted successfully.' });
    } catch (err: any) {
      setStatus({ type: 'error', message: `Failed to delete batch: ${err.message || err}` });
    }
  };
      ...
      complete: (results) => {
        try {
          const students: Student[] = results.data.map((row: any) => ({
            ...
            year_enrolled: Number(row.year_enrolled) || new Date().getFullYear(),
            shs_name: row.shs_name?.trim() || null,
            status_passed_sf10: ['true', '1', 'yes', 'passed'].includes(String(row.status_passed_sf10).toLowerCase().trim())
          })).filter(s => s.student_id && s.last_name && s.first_name && s.program);
          
          setParsedData(students);
          if (students.length === 0) {
            setStatus({ type: 'error', message: 'No valid records found in CSV. Check column headers.' });
          }
        } catch (err) {
          console.error(err);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: `Successfully imported ${count} student records.` });
      setFile(null);
      setParsedData([]);
      if (fileInputRef.current) fileInputRef.current.value = '';
      fetchBatches();
  ...
  const expectedHeaders = "student_id, last_name, first_name, middle_name, program, year_enrolled, shs_name, status_passed_sf10";
      ...
      <div className="bg-white dark:bg-neutral-950 rounded-2xl shadow-sm border border-neutral-200 dark:border-neutral-800 p-8">
        <div className="space-y-4 mb-8">
          ...
          <p className="text-neutral-500 text-sm leading-relaxed">
            Please ensure your Excel spreadsheets are converted to <strong className="text-neutral-900 dark:text-neutral-100">.csv</strong> format before uploading. 
            The system expects the following exact column headers (case-sensitive):
          </p>
        ...
        <div className="relative border-2 border-dashed border-neutral-300 dark:border-neutral-700 hover:border-primary-500 dark:hover:border-primary-500 rounded-2xl p-12 transition-colors flex flex-col items-center justify-center text-center group bg-neutral-50/50 dark:bg-neutral-900/50 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
          <input 
            type="file" 
            accept=".csv"
          ...
          <h4 className="text-lg font-bold mb-1 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">Select a CSV file to upload</h4>
          <p className="text-neutral-500 text-sm">Or drag and drop your file here</p>
        </div>
          ...
          <div className="mt-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 shadow-inner">
              ...
              <div className="flex-1">
                <p className="font-bold text-sm text-neutral-900 dark:text-white mb-1">{file.name}</p>
                <div className="flex gap-3 text-xs font-semibold text-neutral-500">
                  <span className="bg-neutral-200 dark:bg-neutral-800 px-2.5 py-1 rounded-full text-neutral-700 dark:text-neutral-300">{(file.size / 1024).toFixed(2)} KB</span>
                  <span className={parsedData.length > 0 ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-2.5 py-1 rounded-full" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 px-2.5 py-1 rounded-full"}>
                    ...
                    <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800 bg-white dark:bg-neutral-950/50">
                      {parsedData.map((student, i) => (
                        <tr key={i} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50">
                          <td className="px-4 py-2 font-medium">{student.student_id}</td>
                          <td className="px-4 py-2">{student.last_name}, {student.first_name}</td>
                          <td className="px-4 py-2">{student.program}</td>
                          <td className="px-4 py-2">{student.year_enrolled}</td>
                          <td className="px-4 py-2">
                            <span className={student.status_passed_sf10 ? "text-green-600 dark:text-green-400" : "text-red-500"}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                  ...
                  <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                    {batches.map((batch) => (
                      <tr key={batch.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                        <td className="px-6 py-4 text-neutral-500 whitespace-nowrap">
                          {new Date(batch.imported_at).toLocaleString()}
                        </td>
                        <td className="px-6 py-4 font-medium">{batch.file_name}</td>
                        ...
                        <td className="px-6 py-4 text-right">
                          {confirmDeleteId === batch.id ? (
                            <div className="flex items-center justify-end gap-2 animate-in fade-in slide-in-from-right-4 duration-300">
                              <span className="text-xs text-red-500 font-semibold mr-2">Delete all?</span>
                              <button
